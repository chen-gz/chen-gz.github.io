...
We only need to adjust `total_ans` by considering the edges connected to `v`.
 ...
 and 𝑣𝑖
 are the endpoints of the edge, and 𝑐𝑖
 is the edge parameter. The cost of the edge is defined as follows: if the colors of vertices 𝑢𝑖
 and 𝑣𝑖
 are the same, the cost is 0
; otherwise, the cost is 𝑐𝑖
.

You are also given 𝑞
 queries. Each query has the form: repaint vertex 𝑣
 to color 𝑥
